...
func (c Client) DirectPayment(request RequestDirectMaster) (res Response, err error) {
	url, _ := url.Parse(fmt.Sprintf("%s/api/v2/payment/direct", c.EnvApi))
	jsonBody, _ := json.Marshal(request)
	signature := fmt.Sprintf("%s", GenerateSignature(string(jsonBody), "POST", c))
	api, err := c.CallApi(url, signature, jsonBody)
...
func (c Client) DirectPaymentCreditCard(request RequestDirectCreditCard) (res Response, err error) {
	url, _ := url.Parse(fmt.Sprintf("%s/api/v2/payment/direct", c.EnvApi))
	jsonBody, _ := json.Marshal(request)
	signature := fmt.Sprintf("%s", GenerateSignature(string(jsonBody), "POST", c))
	api, err := c.CallApi(url, signature, jsonBody)
